...
try:
    viz = Visdom(server=serverIp,port=serverPort,env=serverEnv)
    print('222')
    startup_sec = 1
    ...
    viz.scatter(
        X=np.random.rand(100,3),# 如果是100x3，那就是100个3坐标轴的点，如果是100x2，那就是100个xy坐标轴的点，且只能是2维或3维，不支持更高维
        Y=(Y + 1.5).astype(int),
        opts=dict(
            legend=['Men', 'Women'],
            markersize=5,
